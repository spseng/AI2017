...
class RandomMoveBot():
    ...
    def random_gen(self):
        self.left_power = random.randint(-100, 100)
        self.right_power = random.randint(-100, 100)
    ...
    def execute_state(self):
        ...
        if self.state == "loop":
            ...
            self.turn_right()
            if self.time_since(self.right_start) >= self.TURN_TICK:
                self.state = "loop"
        elif self.state == "forward":
            self.forward(80)
            if self.time_since(self.forward_start) >= self.BACK_TICK:
                self.state = "left"
    ...
    def check_inputs(self):
        ...
        elif self.bump_back.is_on():
            self.state = "forward"
            self.forward_start = self.tick_count
